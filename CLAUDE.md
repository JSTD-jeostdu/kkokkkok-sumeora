...
## 크레딧 절약 개발 방침 (절대 준수, 2026-07-23)
- 이 프로젝트에서 작업할 때는 **크레딧 소모를 항상 신경 쓴다.**
- **서브에이전트(구현자·리뷰어·최종 브랜치 리뷰 등)를 여러 개 동원하는 워크플로우, 또는 opus 등 비싼 모델로 리뷰하는 작업, 실제 Firebase 프로젝트에 여러 번 왕복하는 QA는 시작 전에 반드시 먼저 알린다** — "이 방식은 크레딧이 많이 듭니다" + 이유.
- 작업 규모·리스크에 따라 판단:
  - **작고 리스크 낮은 변경**(기존 배포된 Firestore 문서 포맷을 안 건드리고, 되돌리기 쉬운 것) → 서브에이전트 없이 직접 Read/Edit로 처리.
  - **크고 리스크 있는 변경**(다중 태스크, 이미 배포된 데이터 하위호환 필요, 실제 서버 왕복 검증 필요) → 무거운 워크플로우(subagent-driven-development 등)를 제안하되, 왜 그 비용을 들일 가치가 있는지 먼저 설명하고 진행 여부를 확인한다.
- 리뷰 단계도 항상 최종 opus 리뷰를 붙이지 말고, 작업 규모에 맞는 가장 가벼운 검토 방식을 우선 제안한다.
- 크레딧을 절약할 수 있는 대안이 있다면 항상 먼저 제안한다.
