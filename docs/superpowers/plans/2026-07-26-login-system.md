...
- **(최우선) 다른 기기에서 이미 로그인해본 적 있는 구글 계정으로, 아직 로그인 안 한 새 기기·시크릿창에서 로그인** → `mergeAndSignIn`의 "이미 사용 중" 병합 경로가 실제로 타는 유일한 시나리오. 두 기기의 스테이지가 병합되어 모두 보이는지 확인. 만약 "계정 전환 중 문제가 생겼어요. 다시 로그인해 주세요" 알림이 뜨면 안내대로 다시 로그인해서 정상 복구되는지도 함께 확인(코드 리뷰에서 구글 credential을 보조 앱→기본 앱에 두 번 재사용하는 부분이 실기기에서 검증된 적 없다는 지적이 있었음 — 재시도로 복구되면 정상)
